...
export async function getResources(req) {
  ...
  const result = await pool
    .request()
    .query("SELECT * FROM Resource ORDER BY ResourceName ASC");
...
export async function updateResource(req) {
  ...
  await pool
    ...
    .input("aq", sql.Int, AvailableQuantity)
    .query(
      "UPDATE Resource SET TotalQuantity=@tq, AvailableQuantity=@aq WHERE ResourceID=@id",
    );
...
export {
  getResources as GET,
  createResource as POST,
  updateResource as PATCH,
