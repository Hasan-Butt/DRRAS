...
export async function createDisaster(req) {
  ...
  // Step 2: Link to location if provided
  if (LocationID) {
    await pool
      .request()
      .input("did", sql.Int, disaster.DisasterID)
      .input("lid", sql.Int, Number(LocationID))
      .query(
        `INSERT INTO DISASTER_LOCATION (DisasterID, LocationID) VALUES (@did, @lid)`,
      );
  }
...
export async function updateDisaster(req) {
  ...
  if (!DisasterID)
    return Response.json({ error: "DisasterID required" }, { status: 400 });
  ...
  if (SeverityLevel) {
    fields.push("SeverityLevel = @SeverityLevel");
    request.input("SeverityLevel", sql.Int, SeverityLevel);
  }
  if (Title) {
  ...
  if (Description !== undefined) {
    ...
    request.input("Desc", sql.VarChar, Description);
  }
  ...
  await request.query(
    `UPDATE DISASTER SET ${fields.join(", ")} WHERE DisasterID = @id`,
  );
...
export {
  getDisasters as GET,
  createDisaster as POST,
  updateDisaster as PUT,
  deleteDisaster as DELETE,
};
