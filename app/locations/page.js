...
const STATUS_STYLES = {
  Critical: {
    bar: "bg-[#ba1a1a]",
    badge: "bg-[#ba1a1a]/15 text-[#ba1a1a]",
    text: "text-[#ba1a1a]",
  },
  Elevated: {
    bar: "bg-[#00687a]",
    badge: "bg-[#00687a]/15 text-[#00687a]",
    text: "text-[#00687a]",
  },
  Stable: {
    bar: "bg-[#006229]",
    badge: "bg-[#006229]/15 text-[#006229]",
    text: "text-[#006229]",
  },
};
...
function LocationContent() {
  ...
  const critical = locations.filter(
    (l) => STATUS_LEVELS(l.ActiveIncidents) === "Critical",
  ).length;
  const elevated = locations.filter(
    (l) => STATUS_LEVELS(l.ActiveIncidents) === "Elevated",
  ).length;
  const stable = locations.filter(
    (l) => STATUS_LEVELS(l.ActiveIncidents) === "Stable",
  ).length;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl p-6 shadow-ambient col-span-1 md:col-span-2 relative overflow-hidden border border-[#c3c6d7]/20">
              ...
              <div className="flex items-baseline gap-3 mt-1">
                <span className="text-5xl font-extrabold text-[#191c1e]">
                  {loading ? "—" : locations.length}
                </span>
                <span className="text-[#434655] text-sm font-medium">
                  Active Zones
                </span>
              </div>
              <div className="mt-5 flex gap-6">
                {[
                  ...
                  { label: "Stable", val: stable, color: "text-[#006229]" },
                ].map(({ label, val, color }, i) => (
                  <div
                    key={label}
                    className={`flex flex-col ${i > 0 ? "pl-6 border-l border-[#e6e8ea]" : ""}`}
                  >
                    <span className="text-xs text-[#434655] uppercase tracking-wider mb-1">
                      {label}
                    </span>
            ...
            <div className="bg-white rounded-xl p-6 shadow-ambient border border-[#c3c6d7]/20 flex flex-col justify-between">
              ...
              <div className="mt-4 w-full bg-[#e6e8ea] rounded-full h-2">
                <div
                  className="bg-gradient-to-r from-[#004ac6] to-[#57dffe] h-2 rounded-full"
                  style={{
                    width: locations.length
                      ? `${Math.min((stable / locations.length) * 100, 100)}%`
                      : "0%",
                  }}
                ></div>
              </div>
              <p className="text-xs text-[#434655] mt-2 text-right">
                {locations.length
                  ? Math.round((stable / locations.length) * 100)
                  : 0}
                % Stable
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((loc) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`absolute top-0 left-0 w-full h-1 ${st.bar}`}
                    ></div>
                    <div className="flex justify-between items-start mb-5">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${st.badge}`}
                      >
                        {level}
                      </span>
                    </div>
                    <div className="space-y-3 flex-1">
                      {[
                        ...
                        <div
                          key={label}
                          className="flex justify-between items-center py-2 border-b border-[#e6e8ea]/50"
                        >
