...
function ResourcesContent() {
  ...
  const deployedItems = resources.reduce(
    (s, r) => s + ((r.TotalQuantity || 0) - (r.AvailableQuantity || 0)),
    0,
  );
  const lowStock = resources.filter(
    (r) => r.AvailableQuantity < r.TotalQuantity * 0.2,
  ).length;
        ...
        <main className="flex-1 p-8 space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-[#434655] mt-1 text-lg">
                Manage and track all response assets and supplies.
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              ...
              {
                label: "Deployed",
                val: deployedItems.toLocaleString(),
                icon: "local_shipping",
                iconBg: "bg-[#00687a]/15",
              ...
              <div
                key={label}
                className="bg-white rounded-xl p-6 shadow-ambient relative overflow-hidden"
              >
                ...
                <div
                  className={`text-4xl font-black ${label === "Low Stock" && Number(val) > 0 ? "text-[#ba1a1a]" : "text-[#191c1e]"}`}
                >
                  {val}
                </div>
                ...
                <thead>
                  <tr className="bg-[#f2f4f6]/50">
                    {[
                      ...
                      <th
                        key={h}
                        className="py-4 px-6 text-xs font-semibold text-[#434655] uppercase tracking-wider"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="text-sm">
                  {loading ? (
                    ...
                    filtered.map((r, i) => {
                      ...
                      const pct = r.TotalQuantity
                        ? Math.round((used / r.TotalQuantity) * 100)
                        : 0;
                      const isLow = r.AvailableQuantity < r.TotalQuantity * 0.2;
                      return (
                        <tr
                          key={r.ResourceID}
                          className={`border-t border-[#e6e8ea]/50 transition-colors ${i % 2 ? "bg-[#f2f4f6]/20" : ""} hover:bg-[#f2f4f6]/50`}
                        >
                          <td className="py-4 px-6 font-semibold text-[#191c1e]">
                            {r.ResourceName}
                          </td>
                          <td className="py-4 px-6 text-[#434655]">
                            {r.ResourceType}
                          </td>
                          ...
                          <td
                            className={`py-4 px-6 font-medium ${isLow ? "text-[#ba1a1a]" : "text-[#191c1e]"}`}
                          >
                              ...
                              <div className="flex-1 bg-[#e6e8ea] rounded-full h-1.5">
                                <div
                                  ...
                                  style={{
                                    width: `${pct}%`,
                                    background:
                                      pct > 80 ? "#ba1a1a" : "#004ac6",
                                  }}
                                ></div>
                              </div>
                              <span className="text-xs text-[#434655] w-10 text-right">
                                {pct}%
                              </span>
                            </div>
                          </td>
