            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-[#f2f4f6] p-2 rounded-xl">
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 shadow-sm border border-[#c3c6d7]/20">
                  ...
                  >
                    {["All", "Pending", "Approved", "Denied", "Fulfilled"].map(
                      (s) => (
                    ...
                    filtered.map((req, i) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-5">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-bold ${PRIORITY_STYLES[req.PriorityLevel] || ""}`}
                          >
                            {req.PriorityLevel}
                          </span>
                        </td>
                        <td className="py-4 px-5">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1 w-max ${STATUS_STYLES[req.Status] || ""}`}
                          >
                            {req.Status}
                          </span>
                        </td>
                          ...
                          {req.Status === "Pending" && (
                            <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                ...
                                onClick={() =>
                                  handleStatus(req.RequestID, "Approved")
                                }
                              ...
                              <button
                                disabled={updating === req.RequestID}
                                onClick={() =>
                                  handleStatus(req.RequestID, "Denied")
                                }
