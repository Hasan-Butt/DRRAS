          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-[#434655] mt-1 text-lg">
                Monitor, track, and manage all active crises.
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              ...
              >
                ...
                <div
                  className={`text-4xl font-black ${val > 0 && label.includes("Critical") ? "text-[#ba1a1a]" : "text-[#191c1e]"}`}
                >
                  {val}
                </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-[#f2f4f6]/50">
                    {[
                      ...
                      <th
                        key={h}
                        className={`py-4 px-6 text-xs font-semibold text-[#434655] uppercase tracking-wider ${i === 5 ? "text-right" : ""}`}
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="text-sm">
                  {loading ? (
                    ...
                    filtered.map((d, i) => {
                      const sc =
                        SEVERITY_COLORS[d.SeverityLevel] || SEVERITY_COLORS[1];
                      const isActive = d.Status === "Active";
                      return (
                        <tr
                          key={d.DisasterID}
                          className={`group transition-colors border-t border-[#e6e8ea]/50 ${i % 2 ? "bg-[#f2f4f6]/20" : ""} hover:bg-[#f2f4f6]/50`}
                        >
                          <td className="py-4 px-6 font-semibold text-[#191c1e]">
                            <div className="flex items-center gap-3">
                              {isActive && (
                                <span className="w-2 h-2 rounded-full bg-[#ba1a1a] animate-pulse shadow-[0_0_6px_rgba(186,26,26,0.5)]"></span>
                              )}
                              {d.Title}
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-6">
                            <span
                              className={`px-3 py-1 rounded-full text-xs font-bold ${sc.bg} ${sc.text}`}
                            >
                          ...
                          <td className="py-4 px-6 text-right">
                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              {isActive ? (
                                <button
                                  onClick={() =>
                                    handleStatusChange(d.DisasterID, "Resolved")
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    handleStatusChange(d.DisasterID, "Active")
                                  }
